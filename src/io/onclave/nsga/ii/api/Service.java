...
//        }
    ...
    public static boolean dominates(final int competitor1, final int competitor2) {
        ...
        if(((double)objectiveOneValues.get(competitor1) < (double)objectiveOneValues.get(competitor1)) || ((double)objectiveTwoValues.get(competitor1) < (double)objectiveTwoValues.get(competitor1))){
            return false;
        }
...
//        for(int i = 0; i < binaryString.length(); i++) if(binaryString.charAt(i) == '1') value += Math.pow(2, binaryString.length() - 1 - i);
        value = valueBasedOnClustering + (valueBasedOnCalls*4);
